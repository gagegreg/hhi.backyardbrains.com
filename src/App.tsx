...
const UUIDs = {
  ...
  stimAmplitude: 0xbb03,      // UInt8   0-50 mA | 255 = POT
  ...
  wifiStatus: 0xbb0e,
  wifiIP: 0xbb0f,
  currentStimAmplitude: 0xbb10,// RO mirror
...
const writeStr = (c: BluetoothRemoteGATTCharacteristic, s: string) =>
  c.writeValue(encodeStr(s));
...
function App() {
  ...
  const readInitial = async (svc: BluetoothRemoteGATTService) => {
    ...
    try {
      ...
      await r8(UUIDs.stimAmplitude, setStimAmplitude);
  ...
  const saveStimSettings = async () => {
    ...
    await w8(UUIDs.stimAmplitude, stimAmplitude);
              ...
              {operatingMode === 3 && (
                <Box mt={4}>
                  ...
                  <TextField
                    ...
                    margin="normal"
                    type="number"
                    label="Amplitude (mA 0-50, 255=POT)"
                    value={stimAmplitude}
